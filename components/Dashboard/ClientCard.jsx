        ...
        <div className="bg-[#F4F9FF] border border-gray-400 p-4">
          ...
          <div className="sm:flex items-center gap-4">
              <div className="sm:w-24 h-44 sm:h-16 w-full">
                  <Image height={96} width={96} className="object-cover w-full h-full" src={`${process.env.NEXT_PUBLIC_API}/files/download/public/${project?.imageIds[0]}`} alt="" />
              </div>
              <div className="flex py-3 sm:py-0 items-center gap-2">
                  ...
                  <Link href={`/${user?.username}/${user?.userId}`} className="font-bold">{user?.fullName}</Link>
              </div>
          </div>
